...
**`tile_yolo_dataset/tile_yolo_dataset.py`** — splits YOLO train/val[/test] images into overlapping square windows, clips and re-normalizes labels, caps empty tiles, and writes a new YOLO dataset plus optional tile-provenance manifest.
...
- `detect_images/detect_images.py`: tiled inference is default on (`--tiles`); use `--no-tiles` for whole-image inference. It uses `--tile-imgsz 1024`, `--tile-overlap 0.2`, batches all tiles from each source image together at `imgsz=tile`, then merges detections with class-wise NMS (`--tile-iou 0.5`).
